...
def merge_stories(examples):
    ...
    # Handle the last story if dataset doesn't end with EOS token
    if current_story:
        merged_stories.append('\n'.join(current_story) + '\n<|endoftext|>')
    
    return {'text': merged_stories}
...
class TinyStoriesDataModule(L.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        cache_path_train = os.path.join(config.cache_dir, "instruct_chunked_train_dataset.json")
        cache_path_val = os.path.join(config.cache_dir, "instruct_chunked_val_dataset.json")
        ...
        if os.path.exists(cache_path_train) and os.path.exists(cache_path_val):
            ...
            # num = 30
            # i=0
            # for item in merge_stories(dataset['train'][:1000])['text']:
            #     print(item)
            #     print('---')
            #     i+=1
            #     if i>num:
            ...
            tokenized_dataset = merged_dataset.map(
                ...
                batched=True,
                batch_size=1,  # Process one story at a time
                num_proc=None,  # Disable parallel processing for tokenization
            )
